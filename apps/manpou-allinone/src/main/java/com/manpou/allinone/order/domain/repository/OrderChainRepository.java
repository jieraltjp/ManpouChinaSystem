...
/**
 * 订单总览仓库（Phase1 步骤1~4）。
 * 基于 MySQL VIEW v_order_chain_v1（使用 procurement_snapshot 快照明细数据）。
 */
@Repository
public interface OrderChainRepository extends JpaRepository<OrderChainView, Long> {
    ...
    @Query(value = """
        ...
        """,
        countQuery = """
        SELECT COUNT(*) FROM v_order_chain_v1 v
        WHERE (:demandStatus IS NULL OR v.demand_status = :demandStatus)
          AND (:keyword IS NULL
               OR LOWER(v.demand_code) LIKE LOWER(CONCAT('%', :keyword, '%'))
               OR LOWER(v.demand_sub_product_code) LIKE LOWER(CONCAT('%', :keyword, '%'))
    ...
    @Query(value = "SELECT * FROM v_order_chain_v1 WHERE demand_id = :demandId ORDER BY demand_id", nativeQuery = true)
