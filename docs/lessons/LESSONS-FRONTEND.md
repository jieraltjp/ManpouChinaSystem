...
fix: i18n 审计 round 3 — LogisticsPlanPage/QcRecordPage 单位符号 i18n化
...
v1.6.0 后端 `convertToProcurement` API 签名变更，前端 `ProcurementPage.vue` 仍用旧签名调用。
...
QcRecordPage.vue 新规验货弹窗中，`el-input-number`（span 4）在 680px 弹窗内，加减按钮被截断：
...
QcRecordPage.vue 验货弹窗中，两条 `el-divider` 分隔线把连续的验货字段切成孤立区块，破坏表单的整体感。
...
移除 QcRecordPage.vue 新规弹窗中的两条 `el-divider`：
- ❌ 删除 `验货信息` 分隔线
- ❌ 删除 `货物信息` 分隔线
...
| 前端 Vue | `LogisticsPlanPage.vue` | 采购单下拉 → 验货记录下拉 |
...
LogisticsPlanPage.vue 新规调配弹窗，货物长/宽/高三列，`el-input-number` 按钮文字被遮挡。显示"货物长"时部分文字不可见。
...
LogisticsPlanPage.vue：弹窗宽度 640px → 820px，货物尺寸行 span 8 → span 6。
...
QcRecordPage.vue，点"新建"后，`currentRow.value` 仍持有可能的旧记录引用，`onSubmit()` 第 531 行 `if (currentRow.value)` 判断为 `true`，走入 `update()` 分支，`currentRow.value.id` 可能为 `undefined` → 后端 400 或静默失败。
...
- **EV-059**: QcRecordPage 新建保存静默失败 → Lesson 59
