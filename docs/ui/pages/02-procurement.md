...
│          │  共 2 条  [20▼] [◀ 1 ▶]                     │
│          │  ┌─ 详情抽屉（右侧滑出，600px）─────────────┐  │
│          │  │  关联工厂│商品代码│子货号│数量│材质│检测│ │
│          │  │  人民币单价│汇率│票点│估算批发价(JPY)│报关类型│
│          │  │  报关备注（跨列）                        │  │
│          │  │  说明书（跨列）                          │  │
│          │  │  状态（Tag）│客户公司│下单日│厂家出货日│ │
│          │  │  计划出货日│实际出货日│商品担当│日本担当│ │
│          │  │  中国担当│发送目的地（跨列）             │  │
│          │  │  创建时间│更新时间（跨列）               │  │
│          │  │              [关闭]  [编辑]             │  │
│          │  └───────────────────────────────────────┘  │
...
| 详情抽屉：内嵌工厂管理 | 详情抽屉内嵌工厂信息，支持新建/编辑/删除工厂 | ✅ |
| 总览按钮 | 操作列跳转 `/base/overview/procurement/{id}` | ✅ |
...
│   ├── 创建时间（跨列）/ 修改人 / 更新时间
│   └── 底部操作（关闭 / 编辑）
└── 新建/编辑弹窗 (el-dialog, 900px, label-width 88px)
...
详情抽屉内嵌工厂管理功能，在查看发注单详情时可直接操作关联工厂：

| 功能 | 说明 |
|------|------|
| 查看工厂信息 | 显示关联工厂名称、编码、联系人等 |
| 新建工厂 | 详情抽屉内新建关联工厂（POST `/api/v1/factories`） |
| 编辑工厂 | 编辑已有工厂信息 |
| 删除工厂 | 删除工厂（需确认） |

> 详细字段规范见 [11-factory.md](./11-factory.md)。
```
