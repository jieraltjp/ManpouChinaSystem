...
:do_manpou
    ...
    REM Launch
    start "" /MIN cmd /c "java !JAVA_OPTS! -jar "!jar!" --server.port=!ALLINONE_PORT! --spring.profiles.active=local >> "%PROJECT_DIR%\logs\manpou-allinone.log" 2>&1"
    echo !jar! > "%PROJECT_DIR%\logs\manpou-allinone.pid"
...
:do_user
    ...
    start "" /MIN cmd /c "java !JAVA_OPTS! -jar "!jar!" --server.port=!USER_PORT! --spring.profiles.active=local >> "%PROJECT_DIR%\logs\user-service.log" 2>&1"
    echo !jar! > "%PROJECT_DIR%\logs\user-service.pid"
...
:do_web
    ...
    cd /d "!web_dir!"
    start "" /MIN cmd /c "npm run dev >> "%PROJECT_DIR%\logs\web.log" 2>&1"
    echo !web_dir! > "%PROJECT_DIR%\logs\web.pid"
...
:do_status
    ...
    for /f "delims=" %%p in ('powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort !ALLINONE_PORT! -ErrorAction SilentlyContinue ^| Select-Object -First 1 -ExpandProperty OwningProcess" 2^>nul') do (
        echo   [●]  manpou-allinone :!ALLINONE_PORT!  ^(PID=%%p^)
        goto :st_user
    )
    echo   [○]  manpou-allinone :!ALLINONE_PORT!  ^^(not running^^)
    set "_ok=0"
    ...
    for /f "delims=" %%p in ('powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort !USER_PORT! -ErrorAction SilentlyContinue ^| Select-Object -First 1 -ExpandProperty OwningProcess" 2^>nul') do (
        echo   [●]  user-service     :!USER_PORT!  ^(PID=%%p^)
        goto :st_web
    )
    echo   [○]  user-service     :!USER_PORT!  ^^(not running^^)
    set "_ok=0"
    ...
    for /f "delims=" %%p in ('powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort !WEB_PORT! -ErrorAction SilentlyContinue ^| Select-Object -First 1 -ExpandProperty OwningProcess" 2^>nul') do (
        echo   [●]  web             :!WEB_PORT!  ^(PID=%%p^)
        goto :st_done
    )
    echo   [○]  web             :!WEB_PORT!  ^^(not running^^)
    set "_ok=0"
